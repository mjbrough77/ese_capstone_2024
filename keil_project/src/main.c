...
_Noreturn static void eeprom_write_task(void* param){
    ...
    while(1){
        ...
        if(I2C2->SR2 & I2C_SR2_BUSY){
            ...
            eeprom_log.gyro_x_axis = (GyroRead_t)(mpu_data[0]<<8)+(GyroRead_t)mpu_data[1];
            eeprom_log.gyro_y_axis = (GyroRead_t)(mpu_data[2]<<8)+(GyroRead_t)mpu_data[2];
            eeprom_log.gyro_x_axis = (GyroRead_t)(mpu_data[4]<<8)+(GyroRead_t)mpu_data[3];
            xQueueSendToBack(i2c2Q, &eeprom_write_addr, NULL);
            I2C2->CR1 |= I2C_CR1_START;
        }
